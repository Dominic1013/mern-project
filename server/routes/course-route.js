...
router.patch("/:_id", async (req, res) => {
  ...
  let { _id } = req.params; //因為我們要從上面網址拿到_id
  //確認課程存在
  try {
    let courseFound = await Course.findOne({ _id }).exec();
    if (!courseFound) {
      return res.status(400).send("找不到課程。無法更新資料內容");
    }

    //使用者必須是此課程講師，才能編輯課程
    if (courseFound.instructor.equals(req.user._id)) {
      // equals是mongoose裡特有函數，功能和===一樣。
      //編輯課程
...
router.delete("/:_id", async (req, res) => {
  ...
  try {
    let courseFound = await Course.findOne({ _id }).exec();
    if (!courseFound) {
      return res.status(400).send("找不到課程。無法刪除資料內容");
    }
